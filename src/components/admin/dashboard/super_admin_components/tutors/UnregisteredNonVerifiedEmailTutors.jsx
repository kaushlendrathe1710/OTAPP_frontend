...
const UnregisteredNonVerifiedEmailTutors = ({
  isCoAdminComponent,
  isAdminComponent,
}) => {
  ...
  const { data, isLoading, isError, isRefetching, refetch } = useQuery(
    ["UnregisteredNonVerifiedEmailTutors", currentPage],
    () => {
  ...
  async function fetchTutors(page, limit) {
    const res = await api.get(
      `/tutor/get-all-unregistered-tutorList-for-view?page=${page}&limit=${limit}`,
      {
        headers: {
  ...
  return (
    <div className="outletContainer">
      ...
      <h2 className={utilStyles.headingLg}>
        Un-registered Non-verified Email Tutors
      </h2>
        ...
        <table className={glassStyles.table} id="jobTable">
          ...
          >
            <tr>
              ...
              <th>Subjects</th>
              <th>Actions</th>
              {!isCoAdminComponent && <th>Actions</th>}
            </tr>
          </thead>
          <tbody>
            {isRefetching || isSearching ? (
                  ...
                  <tr key={i}>
                    ...
                    {isCoAdminComponent || isAdminComponent ? null : (
                      <td aria-controls="actions">
                        {/* <button className="btnSuccess btn--small" onClick={()=>handleReject(_id)}>Update</button> */}
                        <button
                          className="btnPrimary btn--small"
                          onClick={() =>
                            handleVerifyTutor(_id, () =>
                              setCurrentlyMappedTutors((prev) =>
              ...
              <span className={glassStyles.insideTableLoaderWrapper}>
                <h2>0 Unregistered Non-verified email Tutors</h2>
              </span>
            )}
          </tbody>
        </table>
...
export default UnregisteredNonVerifiedEmailTutors;
