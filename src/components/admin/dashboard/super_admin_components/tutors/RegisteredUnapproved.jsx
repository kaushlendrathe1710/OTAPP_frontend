...
export const RegisteredUnapprovedTutors = ({
  isCoAdminComponent,
  isAdminComponent,
}) => {
  ...
  const { data, isLoading, isError, isRefetching, refetch } = useQuery(
    ["RegisteredUnapprovedTutors", currentPage],
    () => {
  ...
  async function fetchTutors(page, limit) {
    const res = await api.get(
      `/tutor/get-all-unapproved-tutorList-for-view?page=${page}&limit=${limit}`,
      {
        headers: {
  ...
  return (
    <div className="outletContainer">
      ...
      <h2 className={utilStyles.headingLg}>Registered Unapproved Tutors</h2>
        ...
        <table className={glassStyles.table} id="jobTable">
          ...
          <tbody>
            {isRefetching || isSearching ? (
              ...
              <span className={glassStyles.insideTableLoaderWrapper}>
                <h2>0 Un-approved registered Tutors</h2>
              </span>
            )}
          </tbody>
        </table>
