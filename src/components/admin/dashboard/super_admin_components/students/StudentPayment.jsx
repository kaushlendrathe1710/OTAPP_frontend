...
// import { handleUnapprove, handleDeletePermanently } from "./ApprovedStudents";
// import { ConfirmModal } from "../../../../Modal/ConfirmModal";
// import { SubjectsModal } from "../../../../Modal/SubjectsModal";
import MainChat from "../../../../projectChat/MainChat";
import StatusDropDown from "../tutors/StatusDropDown";

export const StudentPayment = ({ isCoAdminComponent, isAdminComponent }) => {
  ...
  useEffect(() => {
    ...
    const weekLabel = `${getFormattedDate(firstDayOfWeek)} - ${getFormattedDate(
      lastDayOfWeek
    )}`;
    setWeek(weekLabel);
  ...
  async function filterData(week) {
    ...
    const data = await api.get(
      `/project/get-data-by-week?start=${weekStart}&end=${weekEnd}`,
      {
        headers: {
  ...
  // useEffect(() => {
  //   setCurrentlyMappedStudents(data?.students);
  //   // console.log("mapped students: ", data?.students);
  ...
  useEffect(() => {
    function getSearchedStudents() {
      ...
      const filterDataByStudentName = filteredData?.filter((data) => {
        return data?.studentName
          ?.toLowerCase()
          ?.includes(searchInput.toLowerCase());
      });

      let allConcatData = filterDataByStudentName.concat(
        filterDataByAssignmentId
      );
      ...
      setCurrentlyMappedStudents(allFilterData);
    }
    if (searchInput !== "") {
      getSearchedStudents();
    } else {
      setCurrentlyMappedStudents(filteredData);
    }
  }, [searchInput]);
  ...
  async function handleStatusChange(status, id) {
    ...
    try {
      const res = await api.patch(
        "/project/update-student-payment-status",
        formData,
        {
    ...
    <>
      {chatStatus && (
        <MainChat
          ...
          onClose={toggleModalP}
          type={selectedProjectType}
        />
      )}
      <div className="outletContainer">
        <h2 className={utilStyles.headingLg}>Student Payment</h2>
              ...
              >
                {[...Array(52)].map((_, i) => {
                  ...
                  const firstDayOfWeek = new Date(
                    ...
                    1 +
                      i * 7 -
                      (new Date(today.getFullYear(), 0, 1).getDay() || 7)
                  );
                  ...
                  const weekLabel = `${getFormattedDate(
                    firstDayOfWeek
                  )} - ${getFormattedDate(lastDayOfWeek)}`;
          ...
          <table className={glassStyles.table} id="jobTable">
            ...
            <tbody>
              {isRefetching ? (
                ...
                currentlyMappedStudents?.map((payment, i) => {
                  return (
                    <tr key={i}>
                      <td>{payment.assignmentId}</td>
                      <td>{payment.studentName}</td>
                      <td>{payment?.sPayment}</td>
                      <td>
                        <div className={`${glassStyles.inputWrapper}`}>
                          <StatusDropDown
                            currentStatus={payment.status}
                            onChange={(newStatus) =>
                              handleStatusChange(newStatus, payment?._id)
                            }
                          />
                        </div>
                      </td>
                      ...
                      <td>
                        <button
                          onClick={() => toggleModalP(payment, "student")}
