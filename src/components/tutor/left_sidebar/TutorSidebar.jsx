...
export const TutorUnderReviewLinks = [
  {
    id: 1,
    path: "personal-details",
    label: "Personal Details",
...
export const TutorSidebar = ({
  ...
  let [allNavLinks, setAllNavLinks] = useState(linksArr);
  ...
  useEffect(() => {
    ...
    if (
      ...
      (window.location.href.includes("review") ||
        window.location.href.includes("block"))
    ) {
      setAllNavLinks(TutorUnderReviewLinks);
      setHome("review");
    } else {
      setAllNavLinks(linksArr);
      setHome("home");
    }
  }, [isReviewed]);
