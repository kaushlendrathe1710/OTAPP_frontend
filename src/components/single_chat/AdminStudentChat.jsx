...
async function getAllConversations(page, limit) {
  const res = await api.post(
    `/admin-student-conversation/get-all-conversations?page=${page}&limit=${limit}`
  );
  return res.data;
}
async function getSubAdminAssignedStudentsConversations(userId) {
  const res = await api.get(
    `/admin-student-conversation/get-subadmin-assigned-students-conversations?subAdminId=${userId}`
  );
  return res.data;
}
async function getSingleConversation(conversationId) {
  const res = await api.get(
    `/admin-student-conversation/get-single-conversation?conversationId=${conversationId}`
  );
  return res.data;
}
async function searchConversationByStudentName(studentName) {
  const res = await api.get(
    `/admin-student-conversation/find-conversation-by-student-name?studentName=${studentName}`
  );
  return res.data;
}
async function fetchMessages(conversation_id, page, limit, customSkip = 0) {
  const res = await api.get(
    `/admin-student-message/get-messages?conversationId=${conversation_id}&page=${page}&limit=${limit}&customSkip=${customSkip}`
  );
...
async function fetchUnreadMessages(conversationId, userId) {
  const res = await api.get(
    `/admin-student-message/get-unread-messages?conversationId=${conversationId}&userId=${userId}`
  );
...
function handleSocketEmitUserJoinConversation(user, socket, conversation) {
  if (user && socket && conversation) {
    socket.emit("intialize_admin_student_conversation");
    socket.emit("admin_student_join_conversation", {
...
function handleSocketEmitUserLeaveConversation(user, socket, conversation) {
  if (user && socket && conversation) {
    socket.emit("admin_student_leave_conversation", {
      conversation_id: conversation?._id,
...
function handleSocketEmitUserStartTyping(user, socket, conversation) {
  if (user && socket && conversation) {
    socket.emit("admin_student_user_start_typing", {
      conversation_id: conversation?._id,
      user: { _id: user?._id, name: user?.name },
    });
  }
...
function handleSocketEmitUserStoppedTyping(user, socket, conversation) {
  if (user && socket && conversation) {
    socket.emit("admin_student_user_stopped_typing", {
      conversation_id: conversation?._id,
      user: { _id: user?._id, name: user?.name },
    });
  }
...
function handleSocketOnUserStartTypingFromServer(
  ...
  if (socket) {
    socket.on(
      "admin_student_user_start_typing_from_server",
    ...
    return () => socket.off("admin_student_user_start_typing_from_server");
  }
}
/**
 *
...
function handleSocketOnUserStoppedTypingFromServer(socket, setTypingUsers) {
  if (socket) {
    socket.on(
      "admin_student_user_stopped_typing_from_server",
      ({ conversation_id, user: typingUser }) => {
        setTypingUsers((prev) =>
    ...
    return () => socket.off("admin_student_user_stopped_typing_from_server");
  }
}
/**
 *
...
function handleSocketOnRecieveMessages(
  ...
  if (socket) {
    socket.on("admin_student_recieve_message", async (messages) => {
      if (messages[0]?.conversation === conversationId) {
        setMessages((prev) => [...messages, ...prev]);
        finalCallback(messages);
      }
    });
    return () => socket.off("admin_student_recieve_message");
  }
}
/**
 *
...
function handleSocketOnReadMessages(socket, userId, setMessages) {
  if (socket && userId) {
    socket.on(
      "admin_student_reads_the_message",
    ...
    return () => socket.off("admin_student_reads_the_message");
  }
}
/**
 *
...
function handleSocketOnUserReactedMessage(socket, messages, setMessages) {
  if (socket && messages) {
    socket.on("admin_student_message_reaction_updated", (data) => {
    ...
    return () => socket.off("admin_student_message_reaction_updated");
  }
}
/**
 *
...
function handleSocketOnDeleteMessages(socket, setMessages) {
  socket.on("admin_student_delete_these_messgaes", (data) => {
  ...
  return () => socket.off("admin_student_delete_these_messgaes");
}
/**
 *
...
async function handleSendMessage(
  ...
  try {
    ...
    let url = replyMessage
      ? `/admin-student-message/reply-message?replyMessageId=${replyMessage._id}`
    ...
    socket.emit("admin_student_send_message", newMessages);
    socket.emit("admin_student_update_latest_message", {
      conversation_id: conversation?._id,
  ...
  if (
    ...
    if (readUserId === senderId) {
      ...
      try {
        ...
        const res = await api.post("/admin-student-message/read-message", {
          messageId: messageId,
        ...
        if (res.data) {
          ...
          socket.emit("admin_student_read_message", {
            message: sm_message,
            message_read_user: res.data,
          });
        }
...
async function handleAddReactionToMessage(
  ...
  if (reaction && socket && messageId && userId && conversationId) {
    try {
      const res = await api.post("/admin-student-message/react-message", {
        messageId: messageId,
        reactionUserId: userId,
        reaction,
      });
      socket.emit("admin_student_reaction_added_to_message", {
        conversation_id: conversationId,
        message_id: messageId,
...
async function handleSendForwardMessage(message, selectedUsers, socket, user) {
  ...
  console.log(updateSelectedMessagesData);
  // console.log("selected Users", selectedUsers, "Students", students);

  const res = await api.post("/admin-student-message/forward-message", {
  ...
  selectedUsers?.forEach((groupId) => {
    ...
    socket.emit("admin_student_send_message", forwardMessages);
    socket.emit("admin_student_update_latest_message", {
  ...
  if (socket && conversationId && userId && selectedMessages) {
    ...
    try {
      const res = await api.patch(
        "/admin-student-message/delete-messages-for-me",
        {
      ...
      if (res.data?.acknowledged) {
        socket.emit("admin_student_delete_messages", data);
        socket.emit("admin_student_update_latest_message", {
          conversation_id: conversationId,
          latestMessage: {
...
async function handleDeleteMessagesForEveryone(
  ...
  if (socket && conversationId && userId && selectedMessages) {
    ...
    try {
      const res = await api.patch(
        "/admin-student-message/delete-messages-for-everyone",
        {
          messageIds: data.deleteMessageIds,
        }
      );
...
export {
  getSingleConversation as AdminStudentChat_getSingleConversation,
  fetchMessages as AdminStudentChat_fetchMessages,
  fetchUnreadMessages as AdminStudentChat_fetchUnreadMessages,
  handleSendMessage as AdminStudentChat_handleSendMessage,
  handleMessageRead as AdminStudentChat_handleMessageRead,
  handleAddReactionToMessage as AdminStudentChat_handleAddReactionToMessage,
  handleDeleteMessagesForMe as AdminStudentChat_handleDeleteMessagesForMe,
  handleDeleteMessagesForEveryone as AdminStudentChat_handleDeleteMessagesForEveryone,
  handleSocketEmitUserJoinConversation as AdminStudentChat_handleSocketEmitUserJoinConversation,
  handleSocketEmitUserLeaveConversation as AdminStudentChat_handleSocketEmitUserLeaveConversation,
  handleSocketEmitUserStartTyping as AdminStudentChat_handleSocketEmitUserStartTyping,
  handleSocketEmitUserStoppedTyping as AdminStudentChat_handleSocketEmitUserStoppedTyping,
  handleSocketOnUserStartTypingFromServer as AdminStudentChat_handleSocketOnUserStartTypingFromServer,
  handleSocketOnUserStoppedTypingFromServer as AdminStudentChat_handleSocketOnUserStoppedTypingFromServer,
  handleSocketOnRecieveMessages as AdminStudentChat_handleSocketOnRecieveMessages,
  handleSocketOnReadMessages as AdminStudentChat_handleSocketOnReadMessages,
  handleSocketOnUserReactedMessage as AdminStudentChat_handleSocketOnUserReactedMessage,
  handleSocketOnDeleteMessages as AdminStudentChat_handleSocketOnDeleteMessages,
  handleSendNotificationToGroupParticipants as AdminStudentChat_handleSendNotificationToGroupParticipants,
};
...
const AdminAdminChat = () => {
  ...
  const [selectedTab, setSelectedTab] = useState(SUB_ADMIN_CHAT_TABS[0]);
  const [searchText, setSearchText] = useState("");
  const [searchedConversations, setSearchedConversations] = useState([]);
  const [allConversations, setAllConversations] = useState([]);
  const [subAdminConversations, setSubAdminConversations] = useState([]);
    ...
    (async function () {
      ...
      try {
        if (user?.userType === "Sub-Admin") {
          let subAdminAssignedConversations =
            await getSubAdminAssignedStudentsConversations(user?._id);
          setSubAdminConversations(subAdminAssignedConversations);
        }
        ...
        console.log(
          "Error occured while fetching admin-student conversations: ",
          err
        );
      } finally {
  ...
  useEffect(() => {
    if (socket && user) {
      socket.on("admin_student_recieve_latest_message", async (data) => {
        const { conversation_id, latestMessage } = data;
        const latestConversation = allConversations.find(
          (item) => item?._id === conversation_id
        );
        if (!latestConversation) {
          let fetchedLatestConversation = await getAllConversations(
            1,
            limitRef.current
          );
          setAllConversations((prev) =>
            uniqBy([fetchedLatestConversation, ...prev], "_id")
          );
        } else {
          ...
          setAllConversations(
            uniqBy([updatedConversation, ...filterData], "_id")
          );
        }
      });
    }
    return () => {
      if (socket) {
        socket.off("admin_student_recieve_latest_message");
      }
    };
  ...
  const handleSearchTextChange = useCallback((text) => {
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      if (text) {
        try {
          const searchedData = await searchConversationByStudentName(text);
  ...
  const renderItem = ({ item: conversation, index, data }) => {
    let listUser = conversation?.student;
    let activeUser = selectedConversation?.student;
    ...
    <div className={chatStyles.chatContainer}>
      <div className={chatStyles.chatSidebar}>
        ...
        {searchText !== "" ? (
          ...
          />
        ) : selectedTab.value === "my-students" ? (
          <ChatNavigationList
            listStyle={{ paddingBottom: "180px" }}
            data={subAdminConversations}
            renderItem={renderItem}
            keyExtractor={keyExtractor}
            ListEmptyComponent={() => <EmptyChatList />}
            ListFooterComponent={() => {
