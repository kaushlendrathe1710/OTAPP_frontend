...
const IPAGroupChat = () => {
  ...
  const [groupIPARoom, setGroupIPARoom] = useState();
  ...
  //         `/group/get-all-groups-in-ipa?page=${1}&limit=${10}`, {
  //         headers: {
  ...
  function joinRoom(groupId) {
    ...
    socket.emit("group_ipa_room_join", groupId);
  }
  ...
  async function getConversation(groupId) {
    ...
    const data = await api
      ...
      .catch((err) => {
        console.log("final checking to ipa group error", err);
      });
    console.log(data);
    let ndata = data.data;
  ...
  async function sendMessage() {
    ...
    if (reply) {
      ...
      const { data } = await api.post(
        `/ipa-group-message/reply?message_id=${reply._id}`,
        formData,
        {
          headers: { Authorization: getAccessToken() },
        }
      ...
      socket.emit("group_ipa_send_message", { room_id: groupIPARoom, data });
    } else {
      const { data } = await api.post("/ipa-group-message/send", formData, {
        headers: { Authorization: getAccessToken() },
      });
      setMessages((prev) => [...prev, ...data]);
      socket.emit("group_ipa_send_message", { room_id: groupIPARoom, data });
  ...
  const handelChange = (e) => {
    ...
    socket.emit("group_ipa_typing_started", groupIPARoom);

    if (typingTimeout) clearTimeout(typingTimeout);

    settypingTimeout(
      setTimeout(() => {
        socket.emit("group_ipa_typing_stopped", groupIPARoom);
      }, 1000)
    );
  };
  ...
  async function getMessages() {
    ...
    let { data } = await api.get(
      `/ipa-group-message/get?conversationId=${
        conversation._id
  ...
  async function getMessagesOnScroll() {
    let { data } = await api.get(
      `/ipa-group-message/get?conversationId=${conversation._id}&page=${pageRef.current}&limit=${limitRef.current}`,
      {
        headers: { Authorization: getAccessToken() },
      }
    );
  ...
  useEffect(() => {
    if (socket) {
      socket.on("group_ipa_room_joined", (data) => {
        setGroupIPARoom(data);
      });
      socket.on("group_ipa_recieve_message", (data) => {
        setMessages((prev) => [...prev, ...data]);
      });
      socket.on("group_ipa_typing_started_from_server", () => {
        s;
        setTyping(true);
      });
      socket.on("group_ipa_typing_stopped_from_server", () => {
        setTyping(false);
      });
      socket.on("group_ipa_get_online_user", (data) => {
      ...
      socket.on("group_ipa_get_offline_user", (data) => {
        setRecentChatUsers((prev) =>
          prev.map((item) => {
            if (item?.group?._id === data) {
              return {
