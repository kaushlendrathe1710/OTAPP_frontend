...
function EditIPATutorGroupDetailModal({
  group,
  ...
  const handleUpdateGroupName = useCallback(async () => {
    ...
    try {
      ...
      // const updatedGroupName = updateGroupName?.tutorIpa?.groupName;
      socket.emit("ipa_group_name_update", {
        group_id: conversation._id,
        updatedGroupName: updatedGroupName?.tutorIpa,
      });
      setConversation((prev) => ({
        ...prev,
        groupName: updatedGroupName?.tutorIpa,
      }));
      setGroupMetaValues((prev) => ({
        ...prev,
        canEditGroupName: false,
    ...
    async (selectedAdmins = []) => {
      ...
      try {
        ...
        const { data: updatedGroup } = await api.post(
          `/ipa-group/add-admins-to-tutor-ipa-group`,
          {
            group_id: conversation._id,
            admin_ids: selectedAdmins.map((a) => a._id),
          }
        );
        socket.emit("tutor_ipa_group_add_new_admins", updatedGroup);
  ...
  const handleAddTutors = useCallback(
    async (selectedTutors = []) => {
      ...
      try {
        ...
        const { data: updatedGroup } = await api.post(
          `/ipa-group/add-tutors-to-tutor-ipa-group`,
          {
            group_id: conversation._id,
        ...
        socket.emit("tutor_ipa_group_add_new_tutors", updatedGroup);
        setConversation(updatedGroup);
        setAddTutorsModalVisibility(false);
      } catch (error) {
        console.log("this error occured while adding admins: ", error);
      ...
      if (admin.userType == "Tutor") {
        try {
          ...
          const { data } = await api.post(
            `/ipa-group/remove-tutor-from-tutor-ipa-group`,
            {
              group_id: conversation._id,
              tutor_id: admin._id,
            }
          );
          socket.emit("ipa_tutor_group_delete_tutor", {
            group_id: conversation._id,
            tutor_id: admin._id,
          });
        } catch (error) {
        ...
        try {
          ...
          const { data } = await api.post(
            `/ipa-group/remove-admin-from-tutor-ipa-group`,
            {
              group_id: conversation._id,
              admin_id: admin._id,
            }
          );
          socket.emit("ipa_tutor_group_delete_admin", {
            group_id: conversation._id,
            admin_id: admin._id,
          });
        } catch (error) {
  ...
  const handleDeleteGroup = useCallback(async () => {
    ...
    try {
      if (window.confirm("Are you sure you want to delete this group?")) {
        ...
        const { data } = await api.post(`/ipa-group/delete-ipa-tutor-group`, {
          group_id: conversation._id,
        });
        socket.emit("ipa_tutor_group_delete_group", conversation._id);
...
export default memo(EditIPATutorGroupDetailModal);
