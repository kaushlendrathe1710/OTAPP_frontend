...
function EditIPAStudentGroupDetailModal({
  group,
  ...
  const { data } = useQuery({
    queryKey: ["ipa-group", initialConversation?._id],
  ...
  useEffect(() => {
    if (data) {
      ...
      setAlreadySelectedStudentIds(
        data?.studentParticipants?.map((a) => a._id)
      );
    }
  ...
  useEffect(() => {
    if (initialConversation) {
      ...
      setAlreadySelectedStudentIds(
        initialConversation?.studentParticipants?.map((a) => a._id)
      );
    }
  }, [initialConversation]);
  ...
  const handleUpdateGroupName = useCallback(async () => {
    ...
    try {
      ...
      const { data: updatedGroupName } = await api.patch(
        `/ipa-group/update-group-name`,
        {
          group_id: conversation._id,
          groupName: groupMetaValues.groupName,
          previousName: conversation.groupName,
        }
      );
      // const updatedGroupName = updateGroupName?.studentIpa?.groupName;
      socket.emit("ipa_group_name_update", {
        group_id: conversation._id,
        updatedGroupName: updatedGroupName?.studentIpa,
      });
      setConversation((prev) => ({
        ...prev,
        groupName: updatedGroupName?.studentIpa,
      }));
      setGroupMetaValues((prev) => ({
        ...prev,
        canEditGroupName: false,
  ...
  const handleUpdateGroupDescription = useCallback(async () => {
    ...
    try {
      ...
      const { data: updatedGroupDescription } = await api.patch(
        `/ipa-group/update-group-description`,
        {
          group_id: conversation._id,
          groupDescription: groupMetaValues.groupDescription,
        }
      );
      socket.emit("ipa_group_description_update", {
        group_id: conversation._id,
    ...
    async (selectedAdmins = []) => {
      ...
      try {
        ...
        const { data: updatedGroup } = await api.post(
          `/ipa-group/add-admins-to-student-ipa-group`,
          {
            group_id: conversation._id,
            admin_ids: selectedAdmins.map((a) => a._id),
          }
        );
        socket.emit("student_ipa_group_add_new_admins", updatedGroup);
  ...
  const handleAddStudents = useCallback(
    async (selectedStudents = []) => {
      ...
      try {
        ...
        const { data: updatedGroup } = await api.post(
          `/ipa-group/add-students-to-student-ipa-group`,
          {
            group_id: conversation._id,
        ...
        socket.emit("student_ipa_group_add_new_students", updatedGroup);
        setConversation(updatedGroup);
        setAddStudentsModalVisibility(false);
      } catch (error) {
        console.log("this error occured while adding admins: ", error);
      ...
      if (admin.userType == "Student") {
        try {
          ...
          const { data } = await api.post(
            `/ipa-group/remove-student-from-student-ipa-group`,
            {
              group_id: conversation._id,
              student_id: admin._id,
            }
          );
          socket.emit("ipa_student_group_delete_student", {
            group_id: conversation._id,
            student_id: admin._id,
          });
        } catch (error) {
        ...
        try {
          ...
          const { data } = await api.post(
            `/ipa-group/remove-admin-from-student-ipa-group`,
            {
              group_id: conversation._id,
              admin_id: admin._id,
            }
          );
          socket.emit("ipa_student_group_delete_admin", {
            group_id: conversation._id,
            admin_id: admin._id,
          });
        } catch (error) {
  ...
  const handleDeleteGroup = useCallback(async () => {
    ...
    try {
      if (window.confirm("Are you sure you want to delete this group?")) {
        ...
        const { data } = await api.post(`/ipa-group/delete-ipa-student-group`, {
          group_id: conversation._id,
        });
        socket.emit("ipa_student_group_delete_group", conversation._id);
...
export default memo(EditIPAStudentGroupDetailModal);
