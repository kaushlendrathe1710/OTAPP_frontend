...
async function fetch_all_admins_by_userTypes(from) {
  let url = `/admin/get-all-admins-by-usertypes?userTypesArray=Super-Admin,Admin,Sub-Admin`;
...
const CreateAdminStudentGroupModal = ({ closeModal, onEndReached }) => {
      ...
      if (selectedAdmins.length < 2) {
        ...
        try {
          ...
          const res = await api.post(
            "/admin-student-group-conversation/create-admin-student-group",
            data
          );
          socket.emit("admin_student_group_create_new_group", res.data);
...
export default memo(CreateAdminStudentGroupModal);
