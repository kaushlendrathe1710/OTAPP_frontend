...
function EditGroupModal({
  ...
  const { data } = useQuery({
    queryKey: ["admin-admin-group-conversation", initialConversation?._id],
  ...
  const handleUpdateGroupName = useCallback(async () => {
    ...
    try {
      ...
      const { data: updatedGroupName } = await api.patch(
        `/admin-admin-group-conversation/update-group-name`,
        {
          group_id: conversation._id,
          groupName: groupMetaValues.groupName,
        }
      );
      socket.emit("admin_admin_group_name_update", {
        group_id: conversation._id,
        updatedGroupName,
      });
  ...
  const handleUpdateGroupDescription = useCallback(async () => {
    ...
    try {
      ...
      const { data: updatedGroupDescription } = await api.patch(
        `/admin-admin-group-conversation/update-group-description`,
        {
          group_id: conversation._id,
          groupDescription: groupMetaValues.groupDescription,
        }
      );
      socket.emit("admin_admin_group_description_update", {
        group_id: conversation._id,
    ...
    async (selectedAdmins = []) => {
      ...
      try {
        ...
        const { data: updatedGroup } = await api.post(
          `/admin-admin-group-conversation/add-admins`,
          {
            group_id: conversation._id,
            admin_ids: selectedAdmins.map((a) => a._id),
          }
        );
        socket.emit("admin_admin_group_add_new_admins", updatedGroup);
    ...
    async (admin) => {
      ...
      try {
        ...
        const { data } = await api.patch(
          `/admin-admin-group-conversation/remove-admin`,
          {
            group_id: conversation._id,
            admin_id: admin._id,
          }
        );
        socket.emit("admin_admin_group_delete_admin", {
          group_id: conversation._id,
          admin_id: admin._id,
        });
      } catch (error) {
  ...
  const handleDeleteGroup = useCallback(async () => {
    ...
    try {
      if (window.confirm("Are you sure you want to delete this group?")) {
        ...
        const { data } = await api.patch(
          `/admin-admin-group-conversation/delete`,
          {
            group_id: conversation._id,
          }
        );
        socket.emit("admin_admin_group_delete_group", conversation._id);
        ...
        >
          ...
          <div className={styles.modalField}>
            <div className={styles.adminTypeSelection}>
              <div className={styles.adminTypeButtons}>
                {canEditGroup && (
                  <button
            ...
            <h3 style={{ fontSize: "1.25rem", marginTop: "0.75rem" }}>
              Admins
            </h3>
            <List
