...
function getTabs(userType) {
  if (!userType) return [];
  return ADMINS_PROJECT_TABS.map(({ key, label }) => {
    return {
      key: key,
      title:
        (userType === USER_TYPES.coAdmin || userType === USER_TYPES.subAdmin) &&
        key === "newAssignment"
          ? "Unapproved"
          : label,
      component: () => <AdminProjects type={key} />,
    };
  });
}

const AdminProjectsOutlet = () => {
...
export default AdminProjectsOutlet;
