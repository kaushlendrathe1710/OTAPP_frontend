...
const AdminAdminGroupMessage = forwardRef(
  (
    {
      chatType,
      group,
    ...
    const readUserIds = useMemo(
      () => message.readBy?.map((readUser) => readUser?.readBy?._id) || [],
      [message]
    );
    ...
    useEffect(() => {
      if (isIntersecting) {
        // console.log(message);
        // final callback will be always the last parameter, otherwise SEE function
        handleMessageRead(
          userId,
          socket,
          senderId,
    ...
    const messageReadStatus = useMemo(() => {
      ...
      let allParticipantsReadMessage =
        message?.readBy?.length === group?.adminParticipants?.length - 1;
...
export default memo(AdminAdminGroupMessage);
