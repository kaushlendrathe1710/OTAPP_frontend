...
const IncommingCall = ({ answerCall, leaveCall, userVideo, myVideo }) => {
	...
	return (
		...
		>
			<Box sx={{ ...style, width: 400 }}>
				...
				{callAccepted && myVideo && userVideo && (
					<>
						<h3>Ongoing Call with {otherUsername}...</h3>
						<h3>My Video</h3>
						<video ref={myVideo} autoPlay muted width={320} height={320} />
						<h3>User Video</h3>
						<video ref={userVideo} autoPlay muted width={320} height={320} />
					</>
				)}
				<Stack sx={{ mt: 2 }} spacing={1} direction="row">
					...
					{callAccepted && (
						<Button onClick={leaveCall} variant="contained">
							End Call
						</Button>
					)}
				</Stack>
