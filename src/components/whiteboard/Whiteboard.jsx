...
const Whiteboard = ({ style }) => {
	...
	// async function fetchWhitebaord(boardId){
	//     const res = await api.get(`whiteboard/${boardId}`);
	...
	useEffect(() => {
		// fetchBoardData()
		// let res = await api.get(`whiteboard/${boardId}`);
		...
		if (boardId) {
			...
			fetchBoardData();
			// console.log('user from whitebaord', user)
		}
		if (boardId && user) {
		...
		return () => {
			socket?.emit("WB_moved_from_board", {
				roomId: boardId,
				user: JSON.stringify({
					id: user?._id,
					role: user?.role,
	...
	function updateShapesOnDb() {
		...
		if (shapes?.length > 0 && whiteboard) {
			...
			console.log("whiteboard not updated");
		}
	}
	...
	function fetchBoardData(id) {
		...
		if (!board_id) {
			...
			board_id = pathnameArr[pathnameArr.length - 1];
		}
		console.log(
			"location",
		...
		console.log("boardId", board_id, id);
		api
			.get(`whiteboard/${board_id}`)
			.then((res) => {
	...
	useEffect(() => {
		...
		socket?.on("WB_board_updated", ({ userId, BoardUsers, roomId }) => {
			// console.log("Board Updated", BoardUsers);
			console.log("whiteboard id", whiteboard);
			fetchBoardData();
			// fetchBoardData(whiteboardList[currentTabIndex]?._id);
			// if (boardId) fetchBoardData(boardId);
		});
		socket?.on("WB_New_User", (room) => {
		...
		socket?.on("WB_User_Disconnect", (room) => {
		...
		return () => {
			socket?.removeListener("WB_board_updated");
			socket?.removeListener("WB_New_User");
			socket?.removeListener("User_Disconnect");
			socket?.removeListener("disconnect");
		};
	}, [socket]);

	return (
		<div className={styles.whiteboardPageBox} style={style}>
			<MsgContextProvider>
				<WhiteboardHeader />
			</MsgContextProvider>
