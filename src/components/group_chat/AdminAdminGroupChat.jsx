...
import AdminAdminGroupMessage from "../message/AdminAdminGroupMessage";
...
async function getAllConversations(page = 1, limit = 15, user_id) {
  ...
  const res = await api.get(
    `/admin-admin-group-conversation/get-all-groups-for-current-user?page=${page}&limit=${limit}&user_id=${user_id}`
  );
  return res.data.conversationGroups;
}
async function getSingleConversation(conversationId) {
  console.log("getSingleConversation");
  const res = await api.get(
    `/admin-admin-group-conversation/get-single-group?group_id=${conversationId}`
  );
  return res.data;
}
async function fetchMessages(conversation_id, page, limit, customSkip = 0) {
  const res = await api.get(
    `/admin-admin-group-message/get-messages?groupId=${conversation_id}&page=${page}&limit=${limit}&customSkip=${customSkip}`
  );
...
async function fetchUnreadMessages(conversationId, userId) {
  ...
  const res = await api.get(
    `/admin-admin-group-message/get-unread-messages?groupId=${conversationId}&userId=${userId}`
  );
...
function handleSocketEmitUserLeaveConversation(user, socket, conversation) {
  if (user && socket && conversation) {
    socket.emit("admin_admin_group_leave_conversation", {
      group_id: conversation?._id,
...
function handleSocketOnUserStartTypingFromServer(
  ...
  if (socket) {
    socket.on(
      "admin_admin_group_user_start_typing_from_server",
    ...
    return () => socket.off("admin_admin_group_user_start_typing_from_server");
  }
}
/**
 *
...
function handleSocketOnUserStoppedTypingFromServer(socket, setTypingUsers) {
  if (socket) {
    socket.on(
      "admin_admin_group_user_stopped_typing_from_server",
      ({ group_id, user: typingUser }) => {
        setTypingUsers((prev) =>
    ...
    return () =>
      socket.off("admin_admin_group_user_stopped_typing_from_server");
  }
}
/**
 *
...
function handleSocketEmitUserStartTyping(user, socket, conversation) {
  if (user && socket && conversation) {
    socket.emit("admin_admin_group_user_start_typing", {
      group_id: conversation?._id,
      user: { _id: user?._id, name: user?.name },
    });
  }
...
function handleSocketEmitUserStoppedTyping(user, socket, conversation) {
  if (user && socket && conversation) {
    socket.emit("admin_admin_group_user_stopped_typing", {
      group_id: conversation?._id,
      user: { _id: user?._id, name: user?.name },
    });
  }
...
function handleSocketOnRecieveMessages(
  ...
  if (socket) {
    socket.on("admin_admin_group_recieve_message", async (messages) => {
      if (messages[0]?.conversation === conversationId) {
        setMessages((prev) => [...messages, ...prev]);
        finalCallback(messages);
      }
    });
    return () => socket.off("admin_admin_group_recieve_message");
  }
}
/**
 *
...
function handleSocketOnReadMessages(socket, userId, setMessages) {
  if (socket && userId) {
    socket.on(
      "admin_admin_group_reads_the_message",
    ...
    return () => socket.off("admin_admin_group_reads_the_message");
  }
}
/**
 *
...
function handleSocketOnUserReactedMessage(socket, messages, setMessages) {
  if (socket && messages) {
    socket.on("admin_admin_group_message_reaction_updated", (data) => {
    ...
    return () => socket.off("admin_admin_group_message_reaction_updated");
  }
}
/**
 *
...
function handleSocketOnDeleteMessages(socket, setMessages) {
  socket.on("admin_admin_group_delete_these_messgaes", (data) => {
  ...
  return () => socket.off("admin_admin_group_delete_these_messgaes");
}
/**
 *
 * @param {any} socket - Socket
...
function handleSocketOnUpdateGroupDescription(socket, setConversation) {
  if (!socket || !setConversation) return;
  socket.on(
    "admin_admin_group_description_updated",
  ...
  return () => socket.off("admin_admin_group_description_updated");
}
/**
 *
 * @param {any} socket - Socket
...
function handleSocketOnNewAdminsAdded(socket, setConversation) {
  if (!socket || !setConversation) return;
  socket.on("admin_admin_group_new_admins_added", (group) => {
    if (!group) return;
    setConversation(group);
  });
  return () => socket.off("admin_admin_group_new_admins_added");
}
/**
 *
 * @param {any} socket - Socket
...
function handleSocketOnAdminGotDeleted(
  ...
  socket.on("admin_admin_group_admin_got_deleted", ({ group_id, admin_id }) => {
  ...
  return () => socket.off("admin_admin_group_admin_got_deleted");
}
/**
 *
 * @param {any} socket - Socket
...
function handleSocketOnGroupGotDeleted(
  ...
  if (!socket) return;
  socket.on("admin_admin_group_got_deleted", (group_id) => {
    if (!group_id) return;
  ...
  return () => socket.off("admin_admin_group_got_deleted");
}
/**
 *
...
async function handleSendMessage(
  ...
  try {
    ...
    let url = replyMessage
      ? `/admin-admin-group-message/reply-message?replyMessageId=${replyMessage._id}`
      : "/admin-admin-group-message/send-message";
    if (replyMessage) {
      url = `/admin-admin-group-message/reply-message?replyMessageId=${replyMessage._id}`;
    } else {
      url = "/admin-admin-group-message/send-message";
    }
    ...
    socket.emit("admin_admin_group_send_message", newMessages);
    socket.emit("admin_admin_group_update_latest_message", {
      group_id: conversation?._id,
  ...
  if (
    ...
    if (readUserId === senderId) {
      ...
      try {
        ...
        const res = await api.post("/admin-admin-group-message/read-message", {
          messageId: messageId,
        ...
        if (res.data) {
          ...
          socket.emit("admin_admin_group_read_message", {
            message: sm_message,
            message_read_user: res.data,
          });
        }
...
async function handleAddReactionToMessage(
  ...
  if (reaction && socket && messageId && userId && conversationId) {
    try {
      const res = await api.post("/admin-admin-group-message/react-message", {
        messageId: messageId,
        reactionUserId: userId,
        reaction,
      });
      socket.emit("admin_admin_group_reaction_added_to_message", {
        group_id: conversationId,
        message_id: messageId,
  ...
  if (socket && conversationId && userId && selectedMessages) {
    ...
    try {
      const res = await api.patch(
        "/admin-admin-group-message/delete-messages-for-me",
        {
      ...
      if (res.data?.acknowledged) {
        socket.emit("admin_admin_group_delete_messages", data);
        socket.emit("admin_admin_group_update_latest_message", {
          group_id: conversationId,
          latestMessage: {
...
async function handleSendForwardMessage(message, selectedUsers, socket, user) {
  ...
  let admins = selectedUsers.map((admin) => admin._id);

  try {
    const res = await api.post("/admin-admin-group-message/forward-messages", {
      conversationIds: admins,
    ...
    selectedUsers?.forEach((groupId) => {
      ...
      socket.emit("admin_admin_group_send_message", forwardMessages);
      socket.emit("admin_admin_group_update_latest_message", {
...
async function handleDeleteMessagesForEveryone(
  ...
  if (socket && conversationId && userId && selectedMessages) {
    ...
    try {
      const res = await api.patch(
        "/admin-admin-group-message/delete-messages-for-everyone",
        {
          messageIds: data.deleteMessageIds,
        }
      );
...
const AdminAdminGroupChat = () => {
  ...
  useEffect(() => {
    if (socket && user) {
      socket.on("admin_admin_group_new_group_created", (newCreatedGroup) => {
      ...
      socket.on(
        "admin_admin_group_admin_got_deleted",
        ({ group_id, admin_id }) => {
          if (!group_id || !admin_id) return;
    ...
    return () => {
      if (socket) {
        socket.off("admin_admin_group_new_group_created");
  ...
  useEffect(() => {
    if (socket) {
      socket.on("admin_admin_group_recieve_latest_message", async (data) => {
    ...
    return () => {
      if (socket) {
        socket.off("admin_admin_group_recieve_latest_message");
    ...
    <div className={chatStyles.chatContainer}>
      ...
      <Outlet
        context={{
          ...
          Message: AdminAdminGroupMessage,
          fetchMessages,
          fetchUnreadMessages,
      ...
      <div>
        {userSelectionModal && (
          <CreateGroupModal closeModal={handleUserSelectionModal} />
        )}
      </div>
    </div>
  );
...
AdminAdminGroupChat.propTypes = {
  user: PropTypes.object,
};

export default AdminAdminGroupChat;
