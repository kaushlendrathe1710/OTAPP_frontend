...
import IPATutorGroupMessage from "../message/IPATutorGroupMessage";
...
async function getAllConversations(page = 1, limit = 15, user_id) {
  ...
  const res = await api.get(
    `/ipa-group/get-all-tutor-ipa-groups-for-current-user?page=${page}&limit=${limit}&user_id=${user_id}`
  );
  console.log(res.data);
  // const res1 = await api.get(`/admin-admin-group-conversation/get-all-groups-for-current-user?page=${page}&limit=${limit}&user_id=${user_id}`);
  return res.data;
}
...
function handleSocketOnUserStoppedTypingFromServer(socket, setTypingUsers) {
  if (socket) {
    ...
    return () => socket.off("ipa_tutor_group_user_stopped_typing_from_server");
  }
}
/**
 *
...
function handleSocketOnRecieveMessages(
  ...
  if (socket) {
    ...
    return () => socket.off("ipa_tutor_group_recieve_message");
  }
}
/**
 *
...
function handleSocketOnNewAdminsAdded(socket, setConversation) {
  if (!socket || !setConversation) return;
  socket.on("tutor_ipa_group_new_admins_added", (group) => {
    if (!group) return;
    setConversation(group);
  });
  return () => socket.off("tutor_ipa_group_new_admins_added");
}
/**
 *
 * @param {any} socket - Socket
...
function handleSocketOnTutorGotDeleted(
  ...
  if (!socket || !setConversation || !userId) return;
  socket.on("ipa_tutor_group_tutor_got_deleted", ({ group_id, tutor_id }) => {
  ...
  return () => socket.off("ipa_tutor_group_tutor_got_deleted");
}
...
function handleSocketOnGroupGotDeleted(
  ...
  if (!socket) return;
  socket.on("ipa_tutor_group_got_deleted", (group_id) => {
    if (!group_id) return;
  ...
  return () => socket.off("ipa_tutor_group_got_deleted");
}
/**
 *
...
function handleSocketOnNewTutorsAdded(socket, setConversation) {
  if (!socket || !setConversation) return;
  socket.on("tutor_ipa_group_new_tutors_added", (group) => {
    if (!group) return;
    setConversation(group);
  });
  return () => socket.off("tutor_ipa_group_new_tutors_added");
}
...
const IPATutorGroupChat = () => {
  ...
  useEffect(() => {
    if (socket && user) {
      socket.on("ipa_new_group_created", (newCreatedGroup) => {
        console.log(newCreatedGroup?.tutor_ipa, "user Id", user);
        let newCreatedGroupAdminParticipants =
          newCreatedGroup?.tutor_ipa?.adminParticipants?.map((item) => item);
      ...
      socket.on(
        "ipa_tutor_group_admin_got_deleted",
        ({ group_id, admin_id }) => {
          if (!group_id || !admin_id) return;
      ...
      socket.on(
        "ipa_tutor_group_tutor_got_deleted",
        ({ group_id, tutor_id }) => {
          if (!group_id || !tutor_id) return;
  ...
  return (
    <div className={chatStyles.chatContainer}>
      ...
      <Outlet
        context={{
          chatType: CHAT_TYPE.ipaTutorGroupChat,
          selectedConversation,
          setSelectedConversation,
          Message: IPATutorGroupMessage,
          fetchMessages,
          fetchUnreadMessages,
...
IPATutorGroupChat.propTypes = {
  user: PropTypes.object,
};

export default IPATutorGroupChat;
