...
async function getAllConversations(page = 1, limit = 15, user_id) {
  ...
  const res = await api.get(
    `/ipa-group/get-all-student-ipa-groups-for-current-user?page=${page}&limit=${limit}&user_id=${user_id}`
  );
...
function handleSocketOnUserStartTypingFromServer(
  ...
  if (socket) {
    ...
    return () => socket.off("ipa_student_group_user_start_typing_from_server");
  }
}
...
function handleSocketOnRecieveMessages(
  ...
  if (socket) {
    ...
    return () => socket.off("ipa_student_group_recieve_message");
  }
}
/**
 *
...
function handleSocketOnAdminGotDeleted(
  ...
  socket.on("ipa_student_group_admin_got_deleted", ({ group_id, admin_id }) => {
  ...
  if (
    ...
    if (readUserId === senderId) {
      ...
      try {
        ...
        const res = await api.post("/ipa-student-group-message/read-message", {
          messageId: messageId,
