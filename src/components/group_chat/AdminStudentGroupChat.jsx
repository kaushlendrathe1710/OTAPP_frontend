...
import AdminStudentGroupMessage from "../message/AdminStudentGroupMessage";
...
// import CreateGroupModal from "../createGroupModals/createAdminStudentGroup";
import { CreateAdminStudentGroup } from "../createGroupModals";

import GroupChatListCard from "../Chat/GroupChatListCard";
import chatStyles from "../../styles/chat.module.scss";
import { FiEdit } from "react-icons/fi";

async function getAllConversations(page = 1, limit = 15, user_id, userType) {
  if (userType == "Student") {
    const res = await api.get(
      `/admin-student-group-conversation/get-all-admin-student-groups-conversation-in-which-current-student-exist?page=${page}&limit=${limit}&student_id=${user_id}`
    );
    return res.data.conversationGroups;
  } else {
    const res = await api.get(
      `/admin-student-group-conversation/get-all-admin-student-groups-conversation-in-which-current-admin-exist?page=${page}&limit=${limit}&admin_id=${user_id}`
    );
    return res.data.conversationGroups;
  }
}

async function getSingleConversation(conversationId, userType) {
  const res = await api.get(
    `/admin-student-group-conversation/get-admin-student-group-conversation-by-id?group_id=${conversationId}`
  );
  return res.data;
...
async function fetchMessages(conversation_id, page, limit, customSkip = 0) {
  const res = await api.get(
    `/admin-student-group-message/get-messages?groupId=${conversation_id}&page=${page}&limit=${limit}&customSkip=${customSkip}`
  );
...
async function fetchUnreadMessages(conversationId, userId) {
  const res = await api.get(
    `/admin-student-group-message/get-unread-messages?groupId=${conversationId}&userId=${userId}`
  );
...
function handleSocketEmitUserJoinConversation(user, socket, conversation) {
  if (user && socket && conversation) {
    socket.emit("admin_student_group_join", {
...
function handleSocketEmitUserLeaveConversation(user, socket, conversation) {
  if (user && socket && conversation) {
    socket.emit("admin_student_group_leave_conversation", {
      group_id: conversation?._id,
...
function handleSocketOnUserStartTypingFromServer(
  ...
  if (socket) {
    socket.on(
      "admin_student_group_user_start_typing_from_server",
    ...
    return () =>
      socket.off("admin_student_group_user_start_typing_from_server");
  }
}
/**
 *
...
function handleSocketOnUserStoppedTypingFromServer(socket, setTypingUsers) {
  if (socket) {
    socket.on(
      "admin_student_group_user_stopped_typing_from_server",
      ({ group_id, user: typingUser }) => {
        setTypingUsers((prev) =>
    ...
    return () =>
      socket.off("admin_student_group_user_stopped_typing_from_server");
  }
}
/**
 *
...
function handleSocketEmitUserStartTyping(user, socket, conversation) {
  if (user && socket && conversation) {
    socket.emit("admin_student_group_user_start_typing", {
      group_id: conversation?._id,
      user: { _id: user?._id, name: user?.name },
    });
  }
...
function handleSocketEmitUserStoppedTyping(user, socket, conversation) {
  if (user && socket && conversation) {
    socket.emit("admin_student_group_user_stopped_typing", {
      group_id: conversation?._id,
      user: { _id: user?._id, name: user?.name },
    });
  }
...
function handleSocketOnRecieveMessages(
  ...
  if (socket) {
    socket.on("admin_student_group_recieve_message", async (messages) => {
      if (messages[0]?.conversation === conversationId) {
        setMessages((prev) => [...messages, ...prev]);
        finalCallback(messages);
      }
    });
    return () => socket.off("admin_student_group_recieve_message");
  }
}
/**
 *
...
function handleSocketOnReadMessages(socket, userId, setMessages) {
  if (socket && userId) {
    socket.on(
      "admin_student_group_reads_the_message",
    ...
    return () => socket.off("admin_student_group_reads_the_message");
  }
}
/**
 *
...
function handleSocketOnUserReactedMessage(socket, messages, setMessages) {
  if (socket && messages) {
    socket.on("admin_student_group_message_reaction_updated", (data) => {
    ...
    return () => socket.off("admin_student_group_message_reaction_updated");
  }
}
/**
 *
...
function handleSocketOnDeleteMessages(socket, setMessages) {
  socket.on("admin_student_group_delete_these_messgaes", (data) => {
  ...
  return () => socket.off("admin_student_group_delete_these_messgaes");
}
/**
 *
 * @param {any} socket - Socket
...
function handleSocketOnUpdateGroupDescription(socket, setConversation) {
  if (!socket || !setConversation) return;
  socket.on(
    "admin_student_group_description_updated",
  ...
  return () => socket.off("admin_student_group_description_updated");
}
/**
 *
 * @param {any} socket - Socket
...
function handleSocketOnNewAdminsAdded(socket, setConversation) {
  if (!socket || !setConversation) return;
  socket.on("admin_student_group_new_admins_added", (group) => {
    if (!group) return;
    setConversation(group);
  });
  return () => socket.off("admin_student_group_new_admins_added");
}
/**
 *
 * @param {any} socket - Socket
...
function handleSocketOnAdminGotDeleted(
  ...
  socket.on(
    "admin_student_group_admin_got_deleted",
    ({ group_id, admin_id }) => {
  ...
  return () => socket.off("admin_student_group_admin_got_deleted");
}
/**
 *
 * @param {any} socket - Socket
...
function handleSocketOnNewStudentsAdded(socket, setConversation) {
  if (!socket || !setConversation) return;
  socket.on("admin_student_group_new_students_added", (group) => {
    if (!group) return;
    setConversation(group);
  });
  return () => socket.off("admin_student_group_new_students_added");
}
/**
 *
 * @param {any} socket - Socket
...
function handleSocketOnStudentGotDeleted(
  ...
  if (!socket || !setConversation || !userId) return;
  socket.on(
    "admin_student_group_student_got_deleted",
    ({ group_id, student_id }) => {
  ...
  return () => socket.off("admin_student_group_student_got_deleted");
}
/**
 *
 * @param {any} socket - Socket
...
function handleSocketOnGroupGotDeleted(
  ...
  if (!socket) return;
  socket.on("admin_student_group_got_deleted", (group_id) => {
    if (!group_id) return;
  ...
  return () => socket.off("admin_student_group_got_deleted");
}
/**
 *
...
async function handleSendMessage(
  ...
  try {
    ...
    let url = replyMessage
      ? `/admin-student-group-message/reply-message?replyMessageId=${replyMessage._id}`
      : "/admin-student-group-message/send-message";
    if (replyMessage) {
      url = `/admin-student-group-message/reply-message?replyMessageId=${replyMessage._id}`;
    } else {
      url = "/admin-student-group-message/send-message";
    }
    ...
    socket.emit("admin_student_group_send_message", newMessages);
    socket.emit("admin_student_group_update_latest_message", {
      group_id: conversation?._id,
    ...
    if (readUserId === senderId) {
      ...
      try {
        ...
        const res = await api.post(
          "/admin-student-group-message/read-message",
          {
            messageId: messageId,
        ...
        if (res.data) {
          ...
          socket.emit("admin_student_group_read_message", {
            message: sm_message,
            message_read_user: res.data,
          });
        }
...
async function handleAddReactionToMessage(
  ...
  if (reaction && socket && messageId && userId && conversationId) {
    try {
      const res = await api.post("/admin-student-group-message/react-message", {
        messageId: messageId,
        reactionUserId: userId,
        reaction,
      });
      socket.emit("admin_student_group_reaction_added_to_message", {
        group_id: conversationId,
        message_id: messageId,
...
async function handleDeleteMessagesForMe(
  ...
  if (socket && conversationId && userId && selectedMessages) {
    ...
    try {
      const res = await api.post(
        "/admin-student-group-message/delete-messages-for-me",
        {
      ...
      if (res.data?.acknowledged) {
        socket.emit("admin_student_group_delete_messages", data);
        socket.emit("admin_student_group_update_latest_message", {
          group_id: conversationId,
          latestMessage: {
...
async function handleDeleteMessagesForEveryone(
  ...
  if (socket && conversationId && userId && selectedMessages) {
    ...
    try {
      const res = await api.post(
        "/admin-student-group-message/delete-messages-for-everyone",
        {
          messageIds: data.deleteMessageIds,
        }
      );
...
async function handleSendForwardMessage(message, selectedUsers, socket, user) {
  ...
  const students = selectedUsers.filter((user) => user._id);

  try {
    const res = await api.post(
      "/admin-student-group-message/forward-messages",
      {
    ...
    selectedUsers?.forEach((groupId) => {
      ...
      socket.emit("admin_student_group_send_message", forwardMessages);
      socket.emit("admin_student_group_update_latest_message", {
...
function handleSendNotificationToGroupParticipants(socket, notificationData) {
  if (notificationData && socket) {
    ...
    console.log("Notification sent to group participants", notificationData);
  }
}

const AdminStudentGroupChat = () => {
  ...
  useEffect(() => {
    (async function () {
      ...
      try {
        ...
        console.log(
          "Error occured while fetching admin-student-group conversations: ",
          err
        );
      } finally {
  ...
  useEffect(() => {
    if (socket && user) {
      socket.on("admin_student_group_new_group_created", (newCreatedGroup) => {
        let newCreatedGroupAdminParticipants =
          newCreatedGroup?.adminParticipants?.map((item) => item?._id);
        let newCreatedGroupStudentParticipants =
          newCreatedGroup?.studentParticipants?.map((item) => item?._id);
        if (
          newCreatedGroupAdminParticipants?.includes(user?._id) ||
          newCreatedGroupStudentParticipants?.includes(user?._id)
        ) {
    ...
    return () => {
      if (socket) {
        socket.off("admin_student_group_new_group_created");
        // socket.off("admin_student_group_new_admins_added");
      }
    };
  ...
  useEffect(() => {
    if (socket) {
      socket.on("admin_student_group_recieve_latest_message", async (data) => {
    ...
    return () => {
      if (socket) {
        socket.off("admin_student_group_recieve_latest_message");
        // socket.off("admin_student_group_name_updated");
        // socket.off("admin_student_group_description_updated");
      }
    };
  ...
  const handleOnEndReachedStudents = useCallback(async () => {
    ...
    <div className={chatStyles.chatContainer}>
      ...
      <Outlet
        context={{
          ...
          Message: AdminStudentGroupMessage,
          fetchMessages,
          fetchUnreadMessages,
      ...
      <div>
        {userSelectionModal && (
          <CreateAdminStudentGroup
            closeModal={handleUserSelectionModal}
            onEndReached={handleOnEndReachedStudents}
          />
        )}
      </div>
    </div>
...
AdminStudentGroupChat.propTypes = {
  user: PropTypes.object,
};

export default AdminStudentGroupChat;
