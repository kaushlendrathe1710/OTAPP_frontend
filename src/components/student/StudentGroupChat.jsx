...
const StudentGroupChat = () => {
  ...
  const [groupStudentRoom, setGroupStudentRoom] = useState();
  ...
  const getRecentChats = async () => {
    const res = await api.get("student-group/get-all-groups?page=1&limit=10", {
  ...
  //         `/group/get-all-groups-in-student?page=${1}&limit=${10}`, {
  //         headers: {
  ...
  async function sendMessage() {
    ...
    if (reply) {
      ...
      const { data } = await api.post(
        `/student-group-message/reply?message_id=${reply._id}`,
        formData,
        {
          headers: { Authorization: getAccessToken() },
        }
      ...
      socket.emit("admin_admin_group_send_message", {
        room_id: groupStudentRoom,
        data,
      });
    } else {
      const { data } = await api.post("/student-group-message/send", formData, {
        headers: { Authorization: getAccessToken() },
      });
      setMessages((prev) => [...prev, ...data]);
      socket.emit("admin_admin_group_send_message", {
  ...
  const handelChange = (e) => {
    ...
    socket.emit("admin_admin_group_typing_started", groupStudentRoom);

    if (typingTimeout) clearTimeout(typingTimeout);

    settypingTimeout(
      setTimeout(() => {
        socket.emit("admin_admin_group_typing_stopped", groupStudentRoom);
      }, 1000)
    );
  };
  ...
  async function getMessagesOnScroll() {
    let { data } = await api.get(
      `/student-group-message/get?conversationId=${conversation._id}&page=${pageRef.current}&limit=${limitRef.current}`,
      {
        headers: { Authorization: getAccessToken() },
      }
    );
          ...
          <div className={styles.SideUserContainer}>
            {clickedButton === "RecentChat"
              ...
              : allGroupData.map((data) => {
                  ...
                  return (
                    <AllUsersCard
                      key={data.student_ipa?._id}
                      data={
                        data?.student_ipa === undefined
                          ? data
                          : data?.student_ipa
                      }
              ...
              messages?.map((msg) => {
                return (
                  <MainChat
                    ...
                    conversationBetween={"student-group"}
                  />
                );
              })
            )}
